...
class WP_FullCalendar {
    ...
    unset($_REQUEST['month']); // não é necessário
    unset($_REQUEST['year']); // não é necessário

    // sanitizar as datas de início e término
    $_REQUEST['start'] = $_POST['start'] = date('Y-m-d', strtotime($_REQUEST['start']));
    $_REQUEST['end'] = $_POST['end'] = date('Y-m-d', strtotime($_REQUEST['end']));

    $args = array(
    	'scope' => array($_REQUEST['start'], $_REQUEST['end']),
    	'owner' => false,
    	'status' => 1,
    	'order' => 'ASC',
        'meta_key' => 'data', // Coloque aqui a chave correta do campo ACF
        'orderby' => 'meta_value', // Ordenar pela chave meta
        'full' => 1,
        'post_type' => 'evento', // Substitua 'evento' pelo slug do tipo de post que representa os eventos
    );
    $args['post_status'] = array('publish'); // Excluirá rascunhos e outros status além de "publicado"



    // Obter o tipo de post e as taxonomias, determinar se estamos filtrando por taxonomia
    $post_types = get_post_types(array('public' => true), 'names');
    $post_type = !empty($_REQUEST['type']) && in_array($_REQUEST['type'], $post_types) ? $_REQUEST['type'] : get_option('wpfc_default_type');
    $args['post_type'] = $post_type;
    $args['post_status'] = 'publish'; // Mostrar somente status publicados
    ...
    // criar nosso próprio loop aqui e alterar a consulta SQL para os intervalos de datas
    function wpfc_temp_filter_where($where = '') {
    	global $wpdb;

    // Coloque o nome da tabela de metadados correta (substitua 'wp_postmeta' se necessário)
    	$meta_table = $wpdb->prefix . 'postmeta';

    	$where .= $wpdb->prepare(
    		" AND CAST($meta_table.meta_value AS DATE) >= STR_TO_DATE(%s, '%Y-%m-%d') AND CAST($meta_table.meta_value AS DATE) < STR_TO_DATE(%s, '%Y-%m-%d')",
    		$_REQUEST['start'],
    		$_REQUEST['end']
    	);
    	return $where;
    ...
    // percorrer cada post e adicioná-los ao array de posts para retornar ao navegador
    while ($the_query->have_posts()) {
    	$the_query->the_post();
    	$color = '#a8d144';
        //$post_date = substr($post->post_date, 0, 10);
        //$post_timestamp = strtotime($post->post_date);
    	$acf_date = get_field('data');
    	...
    	if (empty($item_date_counts[$post_date]) || $item_date_counts[$post_date] < $limit) {
    		...
    		$item = array (
    			...
    			'post_id' => $post->ID,
    			'nonce' => wp_create_nonce('wpfc_item_nonce_' . $post->ID)
    		);
    		...
    		$more_array = array (
    			'title' => get_option('wpfc_limit_txt', 'mais ...'),
            ...
            if (!empty($archive_url) || $post_type == 'post') { // posts têm archives
	...
	public static function qtip_content() {
		$content = '';
		if ( !empty($_REQUEST['post_id']) && !empty($_REQUEST['nonce']) && wp_verify_nonce($_REQUEST['nonce'], 'wpfc_item_nonce_' . $_REQUEST['post_id']) ) {
	...
	public static function calendar( $args = array() ) {
		...
		ob_start();
		?>
		<div class="wpfc-calendar-wrapper"><form class="wpfc-calendar" id="wpfc-calendar-<?php echo esc_attr(rand(0, 99999)); ?>"></form><div class="wpfc-loading"></div></div>
		...
		<script type="text/javascript">
			WPFC.data = {
				...
				foreach ( self::$args as $key => $arg ) {
					echo "'" . esc_js($key) . "' : ";
					echo is_numeric($arg) ? (int) $arg : "'" . esc_js($arg) . "'";
					echo ',';
				}
				?>
			};
			WPFC.month = <?php echo esc_js(self::$args['month']); ?>;
			WPFC.year = <?php echo esc_js(self::$args['year']); ?>;
		</script>
		<?php
...
add_filter('em_org_dev_versions', function( $plugin_slugs ) {
	$plugin_slugs['wp-fullcalendar'] = array(
		'slug' => plugin_basename( __FILE__ ),
		'version' => WPFC_VERSION,
	);
	return $plugin_slugs;
});
